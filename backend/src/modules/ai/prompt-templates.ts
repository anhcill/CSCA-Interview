...
export const defaultPromptTemplates: PromptTemplateDefinition[] = [
  {
    ...
    systemPrompt: [
      ...
      "- Stay strictly within targetMajor. School context may list other majors; never ask subject-matter questions from those other majors.",
      "- The first question must allocate about five minutes for self-introduction and ask for academic background, relevant experience, strengths, and application goals.",
  ...
  },
  {
    ...
    systemPrompt: [
      ...
      "Depth 1: clarify one claim or request one concrete example.",
      "Depth 2: ask for the candidate's role, method, measurable result, or evidence.",
      "Depth 3: challenge a limitation, risk, counterargument, or backup plan.",
      "Use candidateClaims and missingContent to choose exactly one gap to probe.",
      "Use remainingMinutes and requiredTopicsRemaining to avoid spending too long on one topic.",
      "If retryInstruction is present, correct every listed validation failure.",
      "Base the next question on a specific claim, example, gap, trade-off, method, result, or assumption in the candidate's latest answer.",
      "Stay strictly within targetMajor. Ignore unrelated majors that may appear in school context.",
    ...
    userPromptTemplate: [
      ...
      "  \"depthStrategy\": {{json:depthStrategy}},",
      "  \"difficulty\": {{json:difficulty}},",
      "  \"followUpDepth\": {{json:followUpDepth}},",
      "  \"missingContent\": {{json:missingContent}},",
      "  \"ragContext\": {{json:ragContext}},",
      "  \"remainingMinutes\": {{json:remainingMinutes}},",
      "  \"requiredTopicsRemaining\": {{json:requiredTopicsRemaining}},",
      "  \"retryInstruction\": {{json:retryInstruction}},",
      "  \"scholarshipType\": {{json:scholarshipType}},",
      "  \"studyPlan\": {{json:studyPlan}},",
      "  \"targetMajor\": {{json:targetMajor}},",
      "  \"targetSchool\": {{json:targetSchool}}",
  ...
  },
  {
    ...
    systemPrompt: [
      ...
      "- content 25%: relevance, directness, specificity, feasibility, required idea coverage, and a clear main point.",
      ...
      "- Confidence is inferred from wording only. Do not claim anything about voice, tone, intonation, pronunciation, speaking pace, or how decisively the answer was read because no audio evidence is supplied here.",
      ...
      "- feedback: 4-6 concise sentences. Cite or closely paraphrase at least one exact claim from answerText, then name exact missing points and the next rewrite priority.",
      "- Never use generic feedback such as 'good but be more confident' without evidence from answerText.",
      "- Describe textual qualities as wording or expression, never as voice delivery.",
      ...
      "- improvedAnswer: rewrite the user's answer into a stronger interview answer in the same answer language. Do not add numbers, achievements, professor names, laboratories, rankings, or school facts absent from answerText/RAG. Use placeholders only when detail is missing, e.g. [ten du an], [moc thoi gian].",
...
export async function resolvePromptTemplate(taskType: ai_task_type, name: string): Promise<ResolvedPromptTemplate> {
  ...
  try {
    ...
    if (dbTemplate && dbTemplate.version >= fallback.version) {
      return {
        id: dbTemplate.id,
